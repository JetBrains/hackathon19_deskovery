...
pub fn led_control(q: bool) {
    unsafe {
        delegate_led_control(q);
    }
}

pub fn delay_ms(ms: i32) {
    unsafe {
        delegate_delay_ms(ms);
    }
}

pub fn display_bg_control(brighness: i32) {
    unsafe {
        delegate_display_bg_control(brighness);
    }
}

pub fn left_ticks() -> i32 {
    unsafe { delegate_left_ticks() }
}

pub fn radar_range() -> i32 {
    unsafe { delegate_radar_range() }
}

pub fn right_ticks() -> i32 {
    unsafe { delegate_right_ticks() }
}

pub fn fill_screen(color: u16) {
    unsafe { ILI9341_Fill_Screen(color) }
}

pub fn deskovery_motor(pwrLeft: i32, pwrRight: i32, recovery: bool) {
    unsafe {
        delegate_deskovery_motor(pwrLeft, pwrRight, recovery);
    }
}

pub fn system_ticks() -> u32 {
    unsafe { delegate_system_ticks() }
}
